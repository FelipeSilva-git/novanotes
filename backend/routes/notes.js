...
// Helper: set tags for a note (replaces existing)
...
router.post('/', (req, res) => {
  ...
  const result = db
    ...
    .run(title, content, folder_id || null, now, now);
...
router.put('/:id', (req, res) => {
  ...
  ).run(title, content, folder_id || null, now, id);

  if (Array.isArray(tag_ids)) {
    setTagsForNote(id, tag_ids);
  }
...
router.delete('/:id', (req, res) => {
  ...
  // note_tags cascade delete handled by FK
...
${tagsXml}
  ...
  <content><![CDATA[${note.content}]]></content>
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="note-${id}.xml"`
    );
    return res.send(xml);
  }
...
<body>
  <div class="wrapper">
    ...
    <div class="meta">
      ${note.folder_name ? `Folder: ${note.folder_name} &nbsp;|&nbsp; ` : ''}
      Created: ${new Date(note.created_at).toLocaleDateString()} &nbsp;|&nbsp;
      Updated: ${new Date(note.updated_at).toLocaleDateString()}
    </div>
    <div class="tags">${tagPills}</div>
    <div class="content">${note.content}</div>
    <div class="footer">Exported from NovaNotes</div>
  </div>
</body>
</html>`;
  ...
  res.setHeader(
    'Content-Disposition',
    `attachment; filename="note-${id}.html"`
  );
